...
//    size_t obstacles_found = 0;
    ...
    while(!queue_empty(&cells)) {
        ...
        vel_acc += forceFromPheromon * //(float2)mix((float)0.85, (float)1.0, (float)
            random(pos+vel+(float2)delta);
    }


    vel += vel_acc ;//*(float2)(2.)  / obstacles_found;
    // random fluctuation
    float r = (random(pos+vel+(float2)delta) - 0.5) * 0.17453292519943 /*10 deg*/;
    //r = 0.01;
    ...
    if (random(pos+vel+(float2)delta) > 0.7) {
    ...
    if (len == 0) {
        ...
        float a = random(pos+vel+(float2)delta) * M_PI * 2;
